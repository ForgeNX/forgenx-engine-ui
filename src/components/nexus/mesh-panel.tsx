...
function sortMiners(list: MeshMiner[], spec: string): MeshMiner[] {
  ...
  const primary: Record<string, (a: MeshMiner, b: MeshMiner) => number> = {
    ...
    fleet: (a, b) => Number(a.assignment === FLEET_AUTO) - Number(b.assignment === FLEET_AUTO),
  };
      ...
      <div className="mt-4 flex flex-col gap-3">
        {mesh.coins.length < 2 && (
          <div
            ...
            style={{ borderColor: "var(--neon-gold)", background: "color-mix(in oklab, var(--neon-gold) 6%, transparent)" }}
          >
        ...
        <SystemMeshPill
          apps={apps}
          mesh={mesh}
          selected={selected === FLEET_WORKER}
          onSelect={() => onSelect(selected === FLEET_WORKER ? null : FLEET_WORKER)}
        />
          ...
          sortMiners(mesh.miners, sort).map((m) => (
            <MinerPill
              ...
              onToggleSystem={async (on) => {
                ...
                const res = await assignMeshWorker(m.worker, on ? FLEET_AUTO : `${here}:100`);
                refresh();
                return res.ok;
              }}
            />
